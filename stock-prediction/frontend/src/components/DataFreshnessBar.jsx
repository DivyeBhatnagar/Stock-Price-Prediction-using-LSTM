// DataFreshnessBar.jsx — Shows data freshness status and allows manual refresh

import { useState, useEffect, useCallback } from "react";
import {
  RefreshCw, CheckCircle2, AlertTriangle, Clock, Database,
  Wifi, WifiOff, ChevronDown, ChevronUp, Eye
} from "lucide-react";
import {
  getDataFreshness, refreshTicker, getSchedulerStatus
} from "../utils/api";

function StatusDot({ stale }) {
  return (
    <span className={`inline-block h-2 w-2 rounded-full ${stale ? "bg-amber-400 animate-pulse" : "bg-emerald-400"}`} />
  );
}

function timeSince(iso) {
  if (!iso) return "never";
  const diff = (Date.now() - new Date(iso).getTime()) / 1000;
  if (diff < 60)   return `${Math.floor(diff)}s ago`;
  if (diff < 3600) return `${Math.floor(diff / 60)}m ago`;
  if (diff < 86400) return `${Math.floor(diff / 3600)}h ago`;
  return `${Math.floor(diff / 86400)}d ago`;
}

export default function DataFreshnessBar({ ticker, onDataRefreshed }) {
  const [summary, setSummary]       = useState(null);
  const [scheduler, setScheduler]   = useState(null);
  const [refreshing, setRefreshing] = useState(false);
  const [expanded, setExpanded]     = useState(false);
  const [error, setError]           = useState(null);

  // Load freshness + scheduler status
  const loadStatus = useCallback(async () => {
    try {
      const [fresh, sched] = await Promise.all([
        getDataFreshness(),
        getSchedulerStatus(),
      ]);
      setSummary(fresh);
      setScheduler(sched);
      setError(null);
    } catch (e) {
      setError("Could not load data status");
    }
  }, []);

  useEffect(() => { loadStatus(); }, [loadStatus]);

  // Auto-reload every 2 minutes (freshness is cached server-side for 60s)
  useEffect(() => {
    const id = setInterval(loadStatus, 120_000);
    return () => clearInterval(id);
  }, [loadStatus]);

  // Manual refresh handler
  const handleRefresh = async (t) => {
    setRefreshing(true);
    setError(null);
    try {
      await refreshTicker(t || ticker);
      await loadStatus();
      onDataRefreshed?.();
    } catch (e) {
      setError(e.response?.data?.detail || e.message);
    } finally {
      setRefreshing(false);
    }
  };

  // Info for the active ticker
  const tickerInfo = summary?.freshness?.[ticker];

  return (
    <div className="rounded-xl border border-surface-border bg-surface-card">
      {/* ── Compact bar ─────────────────────── */}
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 px-4 py-2.5 text-xs">
        {/* Scheduler status */}
        <div className="flex items-center gap-1.5 text-slate-400">
          {scheduler?.running
            ? <><Wifi className="h-3.5 w-3.5 text-emerald-400" /> <span className="text-emerald-400">Auto-sync ON</span></>
            : <><WifiOff className="h-3.5 w-3.5 text-slate-500" /> <span>Auto-sync off</span></>}
        </div>

        {/* Overall freshness counts */}
        {summary && (
          <div className="flex items-center gap-3 text-slate-400">
            <span className="flex items-center gap-1">
              <Database className="h-3.5 w-3.5" />
              {summary.total_tickers} tickers
            </span>
            <span className="flex items-center gap-1 text-emerald-400">
              <CheckCircle2 className="h-3.5 w-3.5" />
              {summary.fresh_count} fresh
            </span>
            {summary.stale_count > 0 && (
              <span className="flex items-center gap-1 text-amber-400">
                <AlertTriangle className="h-3.5 w-3.5" />
                {summary.stale_count} stale
              </span>
            )}
          </div>
        )}

        {/* Active ticker freshness */}
        {tickerInfo && (
          <div className="flex items-center gap-2 rounded-lg bg-surface px-2.5 py-1">
            <StatusDot stale={tickerInfo.is_stale} />
            <span className="font-medium text-white">{ticker}</span>
            <span className="text-slate-400">
              {tickerInfo.latest_date
                ? <>latest: {tickerInfo.latest_date} · {tickerInfo.total_rows.toLocaleString()} rows</>
                : "no data"
              }
            </span>
            {tickerInfo.days_behind > 0 && (
              <span className="text-amber-400">({tickerInfo.days_behind}d behind)</span>
            )}
          </div>
        )}

        {/* Refresh button */}
        <button
          onClick={() => handleRefresh(ticker)}
          disabled={refreshing}
          className="ml-auto flex items-center gap-1.5 rounded-lg bg-blue-600/20 px-3 py-1 text-blue-400 transition
                     hover:bg-blue-600/30 disabled:opacity-50"
        >
          <RefreshCw className={`h-3.5 w-3.5 ${refreshing ? "animate-spin" : ""}`} />
          {refreshing ? "Updating…" : "Refresh"}
        </button>

        {/* Expand/collapse */}
        <button onClick={() => setExpanded(!expanded)} className="text-slate-400 hover:text-white">
          {expanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
        </button>
      </div>

      {/* ── Error ──────────────────────────── */}
      {error && (
        <div className="border-t border-surface-border px-4 py-2 text-xs text-red-400">
          {error}
        </div>
      )}

      {/* ── Expanded detail panel ──────────── */}
      {expanded && summary && (
        <div className="border-t border-surface-border px-4 py-3">
          {/* Scheduler info */}
          {scheduler && (
            <div className="mb-3 flex flex-wrap gap-4 text-xs text-slate-400">
              <span>Mode: <strong className="text-white">{scheduler.config?.mode}</strong></span>
              {scheduler.config?.mode === "interval" && (
                <span>Every: <strong className="text-white">{scheduler.config?.interval_minutes}min</strong></span>
              )}
              {scheduler.next_run && (
                <span>Next run: <strong className="text-white">{new Date(scheduler.next_run).toLocaleTimeString()}</strong></span>
              )}
              {scheduler.last_run && (
                <span>Last run: <strong className="text-white">{timeSince(scheduler.last_run?.timestamp)}</strong>
                  {scheduler.last_run?.refreshed !== undefined && ` (${scheduler.last_run.refreshed}/${scheduler.last_run.total} OK)`}
                </span>
              )}
            </div>
          )}

          {/* Ticker table */}
          <div className="max-h-48 overflow-y-auto rounded-lg border border-surface-border">
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-surface-card text-slate-400">
                <tr>
                  <th className="px-3 py-1.5 text-left font-medium">Ticker</th>
                  <th className="px-3 py-1.5 text-left font-medium">Latest Date</th>
                  <th className="px-3 py-1.5 text-right font-medium">Rows</th>
                  <th className="px-3 py-1.5 text-left font-medium">Updated</th>
                  <th className="px-3 py-1.5 text-center font-medium">Status</th>
                  <th className="px-3 py-1.5 text-center font-medium">Action</th>
                </tr>
              </thead>
              <tbody className="text-slate-300">
                {Object.entries(summary.freshness || {}).map(([t, f]) => (
                  <tr key={t} className="border-t border-surface-border/50 hover:bg-surface/50">
                    <td className="px-3 py-1.5 font-mono font-medium text-white">{t}</td>
                    <td className="px-3 py-1.5">{f.latest_date || "—"}</td>
                    <td className="px-3 py-1.5 text-right">{f.total_rows?.toLocaleString() || 0}</td>
                    <td className="px-3 py-1.5">{timeSince(f.last_updated)}</td>
                    <td className="px-3 py-1.5 text-center">
                      <StatusDot stale={f.is_stale} />
                    </td>
                    <td className="px-3 py-1.5 text-center">
                      <button
                        onClick={() => handleRefresh(t)}
                        disabled={refreshing}
                        className="text-blue-400 hover:text-blue-300 disabled:opacity-40"
                        title={`Refresh ${t}`}
                      >
                        <RefreshCw className="inline h-3 w-3" />
                      </button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>

          {/* Watchlist info */}
          {summary.watchlist?.length > 0 && (
            <div className="mt-2 flex items-center gap-2 text-xs text-slate-400">
              <Eye className="h-3.5 w-3.5" />
              Watchlist: <span className="text-slate-300">{summary.watchlist.join(", ")}</span>
            </div>
          )}
        </div>
      )}
    </div>
  );
}
